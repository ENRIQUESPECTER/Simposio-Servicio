...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errores)) {
        ...
        if (!in_array($hora_inicio, $disponibles)) {
            $errores[] = "El horario seleccionado ya no está disponible.";
        } else {
            // Iniciar transacción
            ...
            try {
                ...
                if ($id_tipo_actividad) {
                    $stmt = $conexion->prepare("INSERT INTO actividad_evento 
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("iiiisssss", $id_evento, $_SESSION['id_usuario'], $id_tipo_actividad, $id_articulo, $titulo, $resumen, $fecha_evento, $hora_inicio, $hora_fin);
                    $stmt->execute();
                }
