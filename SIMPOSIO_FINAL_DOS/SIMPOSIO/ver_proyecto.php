    ...
    <div class="proyecto-container">
        <div class="proyecto-card">
            ...
            <div class="proyecto-body">
                ...
                <?php
                $patrocinadores = obtener_patrocinadores_proyecto($conexion, $id_proyecto);
                if ($patrocinadores->num_rows > 0):
                ?>
                <div class="info-seccion">
                    <h4><i class="fas fa-hand-holding-heart me-2"></i>Patrocinadores del Proyecto</h4>
                    <div class="row">
                        <?php while ($patro = $patrocinadores->fetch_assoc()): ?>
                            ...
                            <div class="card border-success h-100">
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <div>
                                            <h5 class="card-title mb-0"><?php echo htmlspecialchars($patro['nombre_empresa']); ?></h5>
                                            <small class="text-muted">
                                                <i class="fas fa-user me-1"></i><?php echo htmlspecialchars($patro['nombre'] . ' ' . $patro['apellidos']); ?>
                                            </small>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-12 mb-2">
                                            ...
                                            <a href="mailto:<?php echo $patro['correo']; ?>"><?php echo htmlspecialchars($patro['correo']); ?></a>
                                        </div>
                                        <div class="col-12">
                                            <i class="fas fa-calendar-check text-success me-2"></i>
                                            Patrocinio aceptado: <?php echo date('d/m/Y', strtotime($patro['fecha_respuesta'])); ?>
                                        </div>
                                        <?php if (!empty($patro['comentarios_empresa'])): ?>
                                        <div class="col-12 mt-2">
                                            <i class="fas fa-comment text-success me-2"></i>
                                            <small>"<?php echo nl2br(htmlspecialchars($patro['comentarios_empresa'])); ?>"</small>
                                        </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <?php if (!es_empresa()): ?>
                        <!-- Alumnos y docentes: Volver a la lista de sus proyectos -->
                        <a href="mis_proyectos.php" class="btn-volver">
                            <i class="fas fa-arrow-left me-2"></i>Volver a Mis Proyectos
                        </a>
                    <?php else: ?>
                        <!-- Empresas: Volver al inicio (donde está el dashboard) -->
                        <div class="d-flex gap-2">
                            <a href="index.php" class="btn-volver">
                                <i class="fas fa-arrow-left me-2"></i>Volver al Inicio
                            </a>
                            <a href="patrocinar_proyectos.php" class="btn btn-outline-success">
                                <i class="fas fa-search me-2"></i>Buscar más proyectos
                            </a>
                        </div>
                    <?php endif; ?>
                    ...
                    <div>
                        <?php if (!es_empresa()): ?>
                            <!-- Solo alumnos y docentes pueden editar -->
                            <a href="editar_proyecto.php?id=<?php echo $id_proyecto; ?>" class="btn-editar">
                                <i class="fas fa-edit me-2"></i>Editar
                            </a>
                        <?php else: ?>
                            <!-- Empresas: Botón para patrocinar si aplica -->
                            <?php
                            $id_empresa_actual = null;
                            $stmt = $conexion->prepare("SELECT id_empresa FROM empresa WHERE id_usuario = ?");
                            $stmt->bind_param("i", $_SESSION['id_usuario']);
                            $stmt->execute();
                            $empresa_data = $stmt->get_result()->fetch_assoc();
                            if ($empresa_data) {
                                $id_empresa_actual = $empresa_data['id_empresa'];
                            }
                            
                            $ya_patrocinado = false;
                            if ($id_empresa_actual) {
                                $stmt = $conexion->prepare("SELECT COUNT(*) FROM patrocinios WHERE id_articulo = ? AND id_empresa = ? AND estado != 'rechazado'");
                                $stmt->bind_param("ii", $id_proyecto, $id_empresa_actual);
                                $stmt->execute();
                                $ya_patrocinado = $stmt->get_result()->fetch_row()[0] > 0;
                            }
                            ?>
                            <?php if (!$ya_patrocinado && $proyecto['estado'] == 'aprobado'): ?>
                                <a href="patrocinar_proyectos.php" class="btn btn-success">
                                    <i class="fas fa-hand-holding-usd me-2"></i>Patrocinar este proyecto
                                </a>
                            <?php elseif ($ya_patrocinado): ?>
                                <button class="btn btn-secondary" disabled>
                                    <i class="fas fa-check me-2"></i>Patrocinio Solicitado
                                </button>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
